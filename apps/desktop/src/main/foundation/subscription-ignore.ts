 ...
 *
 * The seam beyond issue 1-01/1-15 (which only built the static `.myenv/` + OS-scope rules)
 ...
 * can `include` the synced `.myenv/` JSON and `self-identify` via `[data].dotden_env_id`
 ...
 * 1. the `.myenv/` rule (dotden metadata is never a managed target, ADR 0024);
...
export const MYENV_IGNORE_RULE = '.myenv/'

/** Synced registry + Workspace placement file paths, as chezmoi `include`s them (source-relative). */
const REGISTRY_INCLUDE = '.myenv/environments.json'
const PLACEMENTS_INCLUDE = '.myenv/workspaces.json'
 ...
 *
 * The subscription block needs no input here — it reads the synced `.myenv/` + `dotden_env_id`
...
/**
 * Render the FULL `.chezmoiignore` body — the single generated file for `.myenv/` + OS scope
 * + per-environment Workspace subscription (issue 1-13).
 *
 * The output is a chezmoi Go-template. Its static prefix carries the header, the `.myenv/`
 ...
 *
 * Template walk-through (mirrors the proven spike, adapted to dotden's real `.myenv/` shape —
...
export function renderSubscriptionIgnore(input: SubscriptionIgnoreInput): string {
  // Static lines (header + `.myenv/` + OS scope). chezmoi forward-slashes ignore patterns
  ...
  const staticLines = [
    ...
    // `.myenv/` ALWAYS first so dotden metadata is never applied (ADR 0024).
    MYENV_IGNORE_RULE,
