...
/**
 * WindowControls / WindowTitleBar — the frameless-window chrome (ADR 0036 `den/` surface).
 *
 * A **bespoke** `den/` component: native OS minimize/maximize/close affordances have no
 * shadcn behavioral equivalent (they drive Electron window actions through the preload
 * bridge, not a DOM primitive), so custom is correct. The renderer owns OS-specific
 * layout; native effects stay behind `window.dotden.window.*`.
 */
