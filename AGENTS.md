...
- **Dev conventions (craft)** — [`docs/conventions.md`](docs/conventions.md). Tests, components, main-process layering, the `src/shared` IPC contract (renderer never imports `main/**`, ADR 0031), renderer feature-folders + scoped den-session store (ADR 0027), comments, lint/format.
