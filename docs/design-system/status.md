...
| [secret-and-errors](./screens/secret-and-errors.md) (1-16)         | ◻ partial | **Offline minimum only**: a new `OfflineBanner.tsx` (the spec's `Banner Offline`, `cloud` glyph + muted tone — never destructive-red) renders the persistent "Offline — changes queued · Will sync when you reconnect" strip between the titlebar and the body when this environment has a push **queued** offline (driven by `den.pushPending`). Already-fetched incoming changes still Apply (only push queues), per the spec note. The secret 2-step flow + the apply-error retry screen in this same spec are **issue 2-03/2-05** and **issue 3-08**, not this slice; the full offline state-surface (titlebar glyph + inspector env row → "Offline") is **3-08**.                                                                                                                                                                                                                                                                        |
