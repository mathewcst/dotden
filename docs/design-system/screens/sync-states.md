# Sync states (Syncing · Up to date · Incoming · Ahead to push)
...
| ---------- | ----------------------- | --------------------------------------------------------- | ------------------ |
| `Syncing`  | info/blue · `sync`      | "Syncing… · Pushing 1 commit · checking for incoming"     | —                  |
...
| `Push`     | amber · `git-commit`    | "1 commit ahead · not yet pushed"                         | **Sync now**       |
...
ahead/attention, green = synced, red = failure — the locked functional-color discipline; **ember stays
...
| **1 ahead to push** (`301:3980`) | `Tone=Push` + **Sync now**           | committed-but-unpushed, so neutralized to a clean/committed view (mirrors the Batch-B committed screen): titlebar → "1 to push"; tree M/A letters + decorations **hidden**; diff header → "Committed · macos-sync-2026-06-14"; the "modified" `StatusTag` + Commit/Discard `Button`s **hidden**; "Last commit" → "just now"; env this-mac → "1 commit ahead" (amber), work-laptop → "Synced just now" (green). |
