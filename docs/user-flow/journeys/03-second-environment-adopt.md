...
| **environment role** | the **adopting** environment (#2…N). It _receives_ the whole Den for the first time; it never _sends_ until after it's registered.                                                                                                                                                                                                                                                                                                                                                                |
...
**Connect** is the _same_ provider-agnostic paste-URL + `git ls-remote` preflight + clone screen
...
- **Fallbacks** — auth failure → credential-error state (retry); offline → `ls-remote` fails fast
  ([journey 06]); the empty-repo case is _not_ this journey (it's [01]).

### 1. Found Den — claim an identity _(`OBContent/FoundDen`)_

After clone, dotden detects the existing Den and asks **who this machine is**. Two choices, with a
deliberate default:

- **Set up as a new environment** _(default)_ — mint a fresh **stable random ID**, label prefilled
  from hostname (editable). Continues to **Pick Workspaces** (Step 2).
- **Reclaim an existing environment** _(secondary)_ — for a reinstall / re-image of a machine
...
- An entry that **synced recently** (inside the freshness window) is flagged
  **"⚠ Looks active — last synced 4m ago"**. It's still selectable, but confirming opens a Dialog:
  _"work-laptop synced 4 minutes ago and may still be running. Reclaiming its identity here can
  corrupt sync between the two machines. Reclaim anyway?"_ — **Keep separate** (default; mints a new
...
### 2. Pick Workspaces — what this machine subscribes to _(`OBContent/PickWorkspaces`)_
...
### 3. Review & Apply — the four-bucket reconciliation _(adopt variant of the [operation surface](../screens/operation-surface.md))_
...
_(Scope? · does a Placement resolve on this OS? · local file exists?)_:

1. **Conflicts** — in-Scope, Placement resolves here, **a local file already exists**. This is the
   _dominant_ shape of adopt (a real second machine already has its own `~/.zshrc`,
...
3. **Needs a path** _(path-incompatible)_ — in-Scope, **but** the File carries explicit per-OS
   **Placement** overrides and **none covers this OS**, so dotden genuinely doesn't know where to
   ...
   - **Set a path** → add this OS's Placement → the File drops into bucket 1 or 2. _(This is the
     "move my Ubuntu `.zshrc` to Windows" case: **same shared content, different per-OS path** —
     the content is what you want; you just tell it where it lands.)_
...
> **separate, deferred** concept ([roadmap]) — if a File genuinely needs _different contents_ here,
> adopt does not solve that; you get the shared content.

**The completeness gate.** "Every File must be _set_" = **no File left undecided**: every
**Conflict** resolved and every **Needs-a-path** File dispositioned (set-path _or_ ignore). Buckets
...
### 4. Done / in sync — registration completes _(Applied · in-sync `230:1392`)_
...
automation level ([ADR 0037]), not a silent force-on. The second environment is the whole _point_
...
| Found Den              | reclaim a _live_-looking entry         | warn Dialog → Keep separate (new id) / Reclaim anyway                |
| Pick Workspaces        | continue (≥1 Workspace)                | Review & Apply                                                       |
| Pick Workspaces        | Den has exactly 1 Workspace            | _(screen skipped)_ → Review & Apply                                  |
...
- **Machine already has its own dotfiles** → the _expected_ case, surfaced as **Conflicts** (bucket
...
1. **Four-bucket Apply list.** The DS Review & Apply screen (`228:1153`) groups **CONFLICTS /
   ...
   > ✅ **Reconciled in Figma (2026-06-21).** The apply list is now `AppPane/ChangeList` `Mode=Apply`
   > (master `826:1562`, shared with daily-receive). Added a **NEEDS A PATH** group (a `ListRow`
   > `State=Warn` row, path + **Set a path** affordance — the Set-a-path/Don't-pull choice resolves in
   > the detail pane, mirroring how Conflicts route there) and a **collapsed OTHER SYSTEMS ONLY**
   > disclosure (chevron + count). Both groups are **hidden via `visible=false` on the non-adopt
   > instances** (Conflict-resolver, Secret-&-errors), shown only on adopt.
2. **Completeness counter + gated Apply.** The surface must render a running **"N items still need
   ...
   > ✅ **Satisfied in Figma (2026-06-21).** `AppPane/Apply` `State=Disabled` (set `833:1633`) carries
   > the gate: a `GateHint` now reads **"N items still need you"** (= conflicts + needs-a-path) with a
   > **disabled Apply** button; subtitle retoned "…once every **item** is resolved."
3. **Reclaim liveness flag.** `OBContent/FoundDen`'s reclaim list must show **per-row last-synced /
   ...
   > ✅ **Reconciled in Figma (2026-06-21).** `OBContent/FoundDen` (`216:71`) gained a hidden-by-default
   > **ReclaimRegistry** block (`867:1608`) — `SelectRow` rows (label · OS · last-synced), the live row
   > flagged with an **amber `Pill` "Looks active"**. Demonstrated on **`02b · Reclaim`** (`870:606`)
   > and **`02c · "Looks active" warning`** (`872:681`): a `Dialog Tone=Destructive` over a scrim —
   > **Keep separate** (safe default, outline) / **Reclaim anyway** (red).
4. **Skip Pick-Workspaces when one Workspace** (and when reclaiming, which inherits subscriptions) —
   ...
   > ◑ **Code/prototype only.** The design is correct as-is — `OBContent/PickWorkspaces` (`217:72`) rows
   > are `SelectRow` **checked by default**; the skip/branch is wizard routing, not a static screen.
