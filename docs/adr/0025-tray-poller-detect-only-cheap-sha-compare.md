...
- **A read error never kills the watcher.** A transient `ls-remote` failure (offline, flaky DNS) is surfaced (never fail silently) and the next tick is still armed — the background loop must never crash the app. The surfaced error carries the **same sanitized, URL-free diagnostics as the connect path** (ADR 0020): `latestRemoteSha` reads a _stored_ Remote URL, so a raw `CommandFailedError` would leak that URL into logs and drop the credential hint. Instead it throws a `RemotePollError` with host/scheme-only diagnostics and an actionable help line — including the hint that a Provider's ambiguous "Repository not found" can mean the user's git credentials simply lack access (e.g. wrong `gh auth` account), not that the repo is gone.
...
- **The automation level is environment-local.** Auto-sync is per-environment by definition (CONTEXT.md), so the level persists in Electron `userData`, never in the synced `.dotden/` (ADR 0024). A missing/corrupt/forward-incompatible settings file falls back to the safe Manual rung — never silently into a more-automated mode.
