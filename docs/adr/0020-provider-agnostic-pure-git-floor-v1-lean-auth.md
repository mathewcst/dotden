...
- Because v1 delegates auth to system git, dotden must preserve the user's git credential environment by default. Preflight must be credential-identical to init: resolve chezmoi's effective `git.command` under the same environment that `chezmoi init <url>` will receive, then run `<git.command> ls-remote <url>` with that environment. Do **not** override `GIT_SSH_COMMAND`, `SSH_AUTH_SOCK`, credential helpers, `HOME`, or GUI askpass paths during preflight/init; WSL, 1Password, Git Credential Manager, Keychain, and `gh auth git-credential` setups often depend on them. Prevent hangs with timeout/cancel UX and sanitized diagnostics, not by disabling the user's configured auth mechanisms.
