...
- `apps/desktop` referenced `electron-updater` as a literal `^6.6.2` that bypassed the catalog; folded back to `catalog:desktop` so the version has a single source of truth.
